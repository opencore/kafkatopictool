...
public class OffsetSetter {
  ...
  public OffsetSetter(Map<String, Properties> clusterPropertiesMap) {
    ...
    for (String clusterName : clusterPropertiesMap.keySet()) {
      ...
      clusterProps.setProperty(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG,
          ByteArrayDeserializer.class.getCanonicalName());
      clusterProps.setProperty(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG,
          ByteArrayDeserializer.class.getCanonicalName());
  ...
  public void setOffsets(File offsetsFile, List<String> clusters, boolean useDate) {
    ...
    clusterToOffsets =
        useDate ? getClusterOffsets(offsetRecords) : getClusterOffsetsFromDates(offsetRecords);
    ...
    for (String cluster : clusterToOffsets.keySet()) {
      ...
      for (String consumerGroup : clusterToOffsets.get(cluster).keySet()) {
        ...
        groupProperties.setProperty(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG,
            ByteArrayDeserializer.class.getCanonicalName());
        groupProperties.setProperty(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG,
            ByteArrayDeserializer.class.getCanonicalName());
  ...
  private Map<String, Map<String, Map<TopicPartition, OffsetAndMetadata>>> getClusterOffsets(
  ...
  private Map<String, Map<String, Map<TopicPartition, OffsetAndMetadata>>> getClusterOffsetsFromDates(
      List<CSVRecord> records) {
    ...
    for (String cluster : consumerMap.keySet()) {
      ...
      for (String consumerGroup : groupToPartitionToDatesMap.keySet()) {
        logger.debug("Looking up offsets for dates on cluster " + cluster + " and consumergroup "
            + consumerGroup);
        Map<TopicPartition, OffsetAndTimestamp> offsetsForTimestamps = consumer
            .offsetsForTimes(groupToPartitionToDatesMap.get(consumerGroup));
        ...
        for (TopicPartition topicPartition : offsetsForTimestamps.keySet()) {
          convertedOffsetsForTimestamps.put(topicPartition,
              new OffsetAndMetadata(offsetsForTimestamps.get(topicPartition).offset()));
        }
