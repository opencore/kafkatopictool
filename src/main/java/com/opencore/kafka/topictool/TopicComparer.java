...
public class TopicComparer {
  ...
  private class PartitionCompareThread implements Callable<PartitionCompareResult> {
    ...
    public PartitionCompareThread(TopicPartition partition, Map<String, String> configs,
    ...
    public PartitionCompareResult call() throws Exception {
      ...
      // Assign consumers to partitions - may fail if topic was deleted in the meantime
      // (fringe case, Exception will be caught and treated when the Future is unwrapped later on
      ...
      logger.debug("Setting start offsets for partition " + partition.get(0).partition() + " to "
          + startAtOffset1 + "/" + startAtOffset1);
      ...
      logger.debug("Setting end offsets for partition " + partition.get(0).partition() + " to "
          + compareUntilOffset1 + "/" + compareUntilOffset2);
      ...
      int emptyPolls2 = 0;
      logger
          .debug("Cleanup policy is set to " + topicConfig.get("cleanup.policy").equals("compact"));
      if (compacted) {
        logger.debug(
            "Skipping _per message_ comparison for " + topicPartition + " due to compacted topic.");
      }
      ...
      while (lastComparedOffset1 < compareUntilOffset1
          && lastComparedOffset2 < compareUntilOffset2) {
        ...
        if (emptyPolls1 > 10 && emptyPolls2 > 10 && topicRecords1.isEmpty() && topicRecords2
            .isEmpty()) {
          logger.debug(
              "Aborting due to too many empty polls on partition " + partition.get(0).partition());
          // We don't have any records left in one of the topics and have polled unsuccessfully
          // three times so we can safely abort processing
        ...
        for (int i = 1; i <= batchSize; i++) {
          ...
          ConsumerRecord<String, String> record1 =
              ((LinkedList<ConsumerRecord<String, String>>) topicRecords1)
                  .poll();
          ConsumerRecord<String, String> record2 =
              ((LinkedList<ConsumerRecord<String, String>>) topicRecords2)
                  .poll();
          logger.trace(
              "Partition " + partition.get(0).partition() + ": comparing offsets - " + record1
                  .offset() + "/" + record2.offset());
          ...
          if (compacted) {
            ...
            if (record1 == null || record2 == null) {
              ...
              if (!compareRecords(record1, record2)) {
                logger.debug(
                    "Mismatch in records: " + record1.toString() + " - " + record2.toString());
                ...
                logger.trace(
                    "Match for partition " + partition.get(0).partition() + " - at offsets "
                        + record1.offset() + "/" + record2.offset());
      ...
      if (compacted) {
        ...
        if (keyRecordMap1.keySet().size() != keyRecordMap2.keySet().size()) {
          logger.debug(
              "Mismatch in number of keys: " + keyRecordMap1.keySet().size() + "/" + keyRecordMap2
                  .keySet().size());
