...
public class TopicManager implements AutoCloseable {
    ...
    public void sync(List<NewTopic> targetTopicList, boolean simulate) {
        ...
        for (NewTopic topic : topicsToCreate) {
            System.out.println(topic.name() + " - Partitions: " + topic.numPartitions() + " - Replication: " +
        ...
        for (String topicToModify : topicsToModify) {
            ...
            if (currentState.numPartitions() > targetState.numPartitions()) {
                ...
                System.out.println(topicToModify + ": increasing partition count from " + currentState.numPartitions() + " to " + targetState.numPartitions());
            ...
            if (currentTopicMap.get(topicToModify).replicationFactor() != targetTopicMap.get(topicToModify).replicationFactor()) {
                ...
                System.out.println(topicToModify + ": changing replication factor from " + currentTopicMap.get(topicToModify).replicationFactor() + " to " + targetTopicMap.get(topicToModify).replicationFactor());
                try {
